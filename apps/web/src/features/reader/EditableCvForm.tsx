...
interface EditableCvFormProps {
  ...
  onSave: (data: CandidateFormData, destino: 'candidato' | 'empleado') => void;
  onCancel: () => void;
  confidenceScore?: number;
  /** True cuando la cedula corresponde a un empleado ya registrado en Rosimar. */
  esEmpleadoExistente?: boolean;
  /** Nombre del empleado existente, para mostrarlo en el selector de destino. */
...
export const EditableCvForm: React.FC<EditableCvFormProps> = ({
  ...
  onCancel,
  confidenceScore,
  esEmpleadoExistente = false,
  empleadoNombre,
}) => {
  const [formData, setFormData] = useState<CandidateFormData>(initialData);
  const [destino, setDestino] = useState<'candidato' | 'empleado'>(() =>
    esEmpleadoExistente ? 'empleado' : 'candidato'
  );
  ...
  return (
    <div className="rounded-lg border border-fog bg-paper p-6">
      ...
      {/* Botones de Accion */}
      <div className="mt-8 pt-4 border-t border-fog flex flex-col sm:flex-row justify-between gap-3">
        {esEmpleadoExistente ? (
          <div className="flex flex-col gap-1">
            <span className="text-xs font-semibold text-ink">Guardar como</span>
            <div className="rounded-lg border border-fog overflow-hidden inline-flex">
              <button
                type="button"
                onClick={() => setDestino('empleado')}
                className={`px-4 py-2 text-xs font-semibold transition-colors ${
                  destino === 'empleado' ? 'bg-signal-blue text-white' : 'bg-paper text-steel hover:text-ink'
                }`}
              >
                Empleado existente
              </button>
              <button
                ...
                className={`px-4 py-2 text-xs font-semibold transition-colors ${
                  destino === 'candidato' ? 'bg-signal-blue text-white' : 'bg-paper text-steel hover:text-ink'
                }`}
              >
                Nuevo candidato
              </button>
            ...
            {empleadoNombre && (
              <span className="text-[11px] text-steel">
                Se actualizara la hoja de vida de {empleadoNombre} en Rosimar (no se creara un duplicado).
              </span>
            )}
          </div>
        ) : (
          <div className="self-center hidden sm:block text-[11px] text-steel">
            Se creara un nuevo candidato en la base de datos.
          </div>
        )}
        ...
        <div className="flex gap-3">
          ...
          <button
            type="button"
            onClick={() => onSave(formData, destino)}
          ...
          >
            {esEmpleadoExistente
              ? destino === 'empleado'
                ? 'Guardar Hoja de Vida del Empleado'
