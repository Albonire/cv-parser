...
export function parseContractText(text: string, layout?: DocumentLayout): ContractFormData {
  ...
  if (!position) {
    const lCargo = documento.lines.find(
      (l) => contieneCargo(l.text) && !esPlantilla(l.text)
    );
    if (lCargo) {
      const limpio = lCargo.text.replace(/^[•*\s-]+/g, '').trim();
      if (limpio.split(/\s+/).length <= 6 && !esPlantilla(limpio) && !/@|mail/i.test(limpio)) {
        position = limpio;
      } else {
        const palabras = limpio.split(/[^A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+/);
        const palabraCargo = palabras.find((p) => contieneCargo(p));
        if (palabraCargo) {
...
function buscarCedulaGenerica(texto: string, employerNit?: string): string | undefined {
  ...
  // 1. Cédula con prefijo explícito (incluyendo variaciones OCR: C.C, CC, CO, C.00, C C)
  const conPrefijoOcr = limpio.match(
    /(?:\bcc\b|c\.?\s?c\.?|co\b|c\.00|cedula|documento(?:\s+de\s+(?:identidad|identificacion))?|identificacion)\s*(?:n[oº°]?\.?|numero)?\s*[:.-]?\s*([0-9oO][0-9oO.\s-]{5,11}[0-9])/i
  );
  if (conPrefijoOcr) {
  ...
  const etiquetada = limpio.match(
    /(?:\bcc\b|cedula|documento(?:\s+de\s+(?:identidad|identificacion))?|identificacion)\s*(?:n[oº°]?\.?|numero)?\s*[:.-]?\s*(\d[\d.\s-]{5,}\d)/i
  );
  if (etiquetada) {
...
function extraerCedulaTrabajador(bloque: DocumentLayout, documento: DocumentLayout): string | null {
  ...
  if (esValido(habitual)) return habitual;
  ...
  return esValido(habitual) ? habitual : null;
}
